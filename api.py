...
# How to fetch Cookie
cookies = {'TS01867a55': '01168561aef446db5e7ab4050b3803e0fb20da99ffa33f5fe457feb07f4a3d8f7c55311cadaa52afc5b5294361930e4bd30b968de6', 'scc-contract-business-gateway': 'ZjIzOWZmZmUtMTAwYy00ZmEwLTlmMDQtNDkzNTUwNDFjODlm', 'sidebarStatus': '0'}
...
def request_dept() -> None:
	url = "https://contract.pgyl.cn/permission/getHasPermissionAuthOrgs"
	data = {"authorizationCodes":["business","common"],"hrOrgId":"","hrOrgProviderId":""}
...
def request_contracts(startTime: str, endTime: str) -> None:
	url = "https://contract.pgyl.cn/scc-contract-business/contract/select"
	data = {"isMainContract":"0","contractStatusList":["2","4"],"listType":"WH","authOrgId":"00001000490002700056","contractClassifyCodeList":["CONTRACT_CLASSIFY01"],"dataType":"contract","authOrgType":"2","viewId":1,"createTimeStart":startTime,"createTimeEnd":endTime,"isOperator":0,"listQueryType":"CHECK","qryScene":"JYF","pageSize":20,"pageNo":1}
	resp = requests.post(url, json=data, headers=COMMON_HEADERS, timeout=10)
	resp.raise_for_status()
	data = resp.json()['data']
...
def request_materials() -> None:
	url = "https://contract.pgyl.cn/scc-contract-business/material/qryMaterialList"
	data = {"pageNo":1,"pageSize":20,"materialName":"","materialCode":"","authOrgId":"00001000490002700056","authOrgType":"2"}
	resp = requests.post(url, json=data, headers=COMMON_HEADERS, timeout=10)
	resp.raise_for_status()
	data = resp.json()['data']
...
def export_specific_contract(contract: dict) -> None:
	url = "https://contract.pgyl.cn/scc-contract-business/contract/qryContractRelevance"

	resp = requests.post(url, json=contract, headers=COMMON_HEADERS, timeout=10)
	resp.raise_for_status()
...
if __name__ == "__main__":
	...
	decade_ago_time = time.strftime("%Y-%m-%d", time.localtime(time.time() - 10 * 365 * 24 * 3600))
